...
config, one stub. Three variations are worth knowing. Each replaces only the steps named, so the
...
**As a Claude Code plugin** — *replaces steps 1 and 5, and is Claude Code only.* This repository is
also a plugin marketplace, so Claude Code can install the package and its skill in one step:

```
/plugin marketplace add uchimata2/handoff-skill
/plugin install handoff@handoff
```

There is no copy to place and no stub to edit: the plugin carries the package with it, and the skill
lists as `handoff:handoff` — namespaced, so it cannot collide with a skill you already have. The
config is still per project and still resolves by core §0's chain, looking first for
`.handoff/config.md` in the project you are working in. The plugin is generated from this
repository's sources, so it ships exactly the files the release asset ships. Every other agent uses
the shapes above.
