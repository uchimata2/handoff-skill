...
> selects **Create** or **Close**; the read path (Resume / Status) lives in `resume.md`, and the
...
2. **Resolve any live handoff** at `handoff_file`. Since the session is being closed, not
   ...
   `processed_<timestamp>` form, in place, as §6.4 — §1). Its content already lives in durable homes — the
